...
export default function Index() {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => {
      setMerged(true);
      playWhooshChime();
      setTimeout(() => setShowCenter(true), 600);
    }, 1500);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
    ...
    <div className="min-h-screen relative overflow-hidden bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
      ...
      <div className="h-screen w-full relative">
        <div
          ...
          onMouseLeave={(e) => {
            e.currentTarget.style.transform = revealed
              ? "translateX(0)"
              : "translateX(-100%)";
          }}
        >
        ...
        <div
          className={`absolute inset-y-0 right-0 ${merged ? "w-[30%]" : "w-1/2"} transition-[width,transform] duration-700 ease-out ${revealed ? "translate-x-0" : "translate-x-full"}`}
        >
          <FarmPanel onCursorNear={() => {}} />
        </div>
        {/* Merge gradient */}
        <div
          className={`absolute inset-0 transition-opacity duration-700 ${merged ? "opacity-100" : "opacity-0"}`}
          style={{
            background:
              "linear-gradient(90deg, rgba(0,240,255,0.15), rgba(76,175,80,0.15))",
          }}
        />
        <CenterPanel
          visible={showCenter}
