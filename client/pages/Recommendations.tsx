...
export default function Recommendations() {
  ...
  const recs = useMemo(
    () => [
      {
        icon: "💧",
        title: "Switch to drip irrigation in 2 days.",
        reason: "Low rainfall trend and better water efficiency.",
      },
      {
        icon: "🌱",
        title: "Apply 20kg Nitrogen in Zone 3.",
        reason: "Soil nitrogen low, improves growth.",
      },
      {
        icon: "🐛",
        title: "Use eco-friendly pesticide spray next week.",
        reason: "Leaf scan found pest signs; neem works well.",
      },
      {
        icon: "🌾",
        title: "Plant maize instead of wheat this season.",
        reason: "Moisture profile favors maize; yield +30% expected.",
      },
    ],
    [],
  );
  ...
  const readAll = () => {
    if (!supports) return;
    for (const r of recs) speak(r.title, { lang: `${lang}-IN` as any });
  };

  return (
    <div
      className="min-h-screen relative overflow-hidden"
      style={{ background: "linear-gradient(120deg,#e6fff5,#e8f5ff)" }}
    >
      <RippleCursor />
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {recs.map((r, i) => (
              <HoloCard
                ...
                onAccept={() =>
                  supports &&
                  speak(r.title + " accepted", { lang: `${lang}-IN` as any })
                }
                onReject={() =>
                  supports &&
                  speak(r.title + " rejected", { lang: `${lang}-IN` as any })
                }
              />
            ))}
          </div>
        ...
        <section className="rounded-2xl border bg-white/80 backdrop-blur p-5">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="rounded-xl border p-4 flex items-start gap-3">
              ...
              <div>
                <div className="font-semibold">Maize</div>
                <div className="text-sm text-gray-600">
                  Yield can be +30% higher in current moisture.
                </div>
              </div>
            </div>
            <div className="rounded-xl border p-4 flex items-start gap-3">
              <div className="text-3xl">🫘</div>
              <div>
                <div className="font-semibold">Pulses</div>
                <div className="text-sm text-gray-600">
                  Require less water → good for dry season.
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="rounded-2xl border bg-white/80 backdrop-blur p-5">
          ...
          <div className="flex flex-wrap gap-3">
            <button className="px-4 py-3 rounded-2xl border bg-white/70 hover:bg-white transition">
              Spray neem-based solution this week.
            </button>
            <button className="px-4 py-3 rounded-2xl border bg-white/70 hover:bg-white transition">
              Trap-based pest control for locusts in Zone 2.
            </button>
          </div>
        </section>
        ...
        <section className="rounded-2xl border bg-white/80 backdrop-blur p-5">
          ...
          <div className="flex items-center gap-3">
            <label className="inline-flex items-center cursor-pointer">
              <input
                ...
                checked={sprinkler}
                onChange={(e) => {
                  setSprinkler(e.target.checked);
                  if (supports)
                    speak(
                      e.target.checked ? t("sprinkler_on") : "Sprinklers off",
                      { lang: `${lang}-IN` as any },
                    );
                }}
              />
          ...
          {sprinkler && (
            <div className="mt-4 h-32 rounded-xl border relative overflow-hidden bg-emerald-50">
              ...
              <div className="absolute inset-x-0 bottom-3 text-center text-emerald-700">
                Sprinklers active for 10 min.
              </div>
            </div>
          )}
        </section>
