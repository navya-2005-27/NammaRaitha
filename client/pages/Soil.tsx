    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        const moisture = Math.min(
          100,
          Math.max(10, Math.round((1 - Math.abs(0.5 - brightness)) * 100)),
        );
        ...
        const sev: "low" | "medium" | "high" =
          Math.abs(r - g) < 15 ? "low" : r > g ? "medium" : "low";
        resolve({
          ph,
          moisture,
          n,
          ...
          leaf: {
            healthy: leafHealthy,
            severity: sev,
            note: leafHealthy ? "Healthy leaf" : "Pest signs detected",
          },
        });
      };
...
export default function Soil() {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      setPlace(name);
      if (supports)
        speak("This is your farm location and today's weather.", {
          lang: `${lang}-IN` as any,
        });
    });
  }, [supports, speak, lang]);

  const onSoilUpload: React.ChangeEventHandler<HTMLInputElement> = async (
    e,
  ) => {
    ...
    setTimeout(async () => {
      ...
      setLoading(false);
      if (supports)
        speak("Soil scan complete. Your soil is healthy.", {
          lang: `${lang}-IN` as any,
        });
    }, 600);
  };

  const onLeafUpload: React.ChangeEventHandler<HTMLInputElement> = async (
    e,
  ) => {
    const f = e.target.files?.[0];
    if (!f) return;
    ...
    setTimeout(async () => {
      const res = await analyzeImage(f);
      setLeafResult(res.leaf);
      setLoading(false);
      if (supports)
        speak(
          res.leaf?.healthy
            ? "Leaf looks healthy."
            : "Your crop leaf shows signs of pest infection.",
          { lang: `${lang}-IN` as any },
        );
    }, 600);
  };
  ...
  const recommendation = useMemo(() => {
    ...
    const advice = lowN
      ? "Add 20kg Nitrogen fertilizer in Zone A."
      : "Maintain current fertilization.";
    const pest =
      leafResult && !leafResult.healthy
        ? " Spray organic pesticide tomorrow evening."
        : "";
    return `${advice}${pest}`;
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <header className="w-full sticky top-0 z-20 backdrop-blur bg-white/70 border-b">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <button
            onClick={() => navigate("/dashboard?lang=" + lang)}
            className="rounded-full border bg-white px-3 py-1.5 hover:shadow"
          >
            ← Dashboard
          </button>
          ...
          <button
            onClick={() =>
              supports &&
              speak("Follow the steps to check your soil and crops.", {
                lang: `${lang}-IN` as any,
              })
            }
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section
          className="rounded-2xl border bg-white/80 backdrop-blur p-5 animate-fade-in"
          ...
          {soilResult && (
            <div className="mt-4 grid sm:grid-cols-5 gap-3">
              <div className="rounded-xl border p-3">
                pH: <b>{soilResult.ph}</b>
              </div>
              <div className="rounded-xl border p-3">
                Moisture: <b>{soilResult.moisture}%</b>
              </div>
              <div className="rounded-xl border p-3">
                N: <b>{soilResult.n}</b>
              </div>
        ...
        >
          ...
          {leafResult && (
            <div className="mt-4 rounded-xl border p-4 flex items-center gap-4">
              ...
              <div>
                <div className="font-semibold">
                  {leafResult.healthy ? "Healthy leaf" : leafResult.note}
                </div>
                <div className="text-sm text-gray-600">
                  Severity: {leafResult.severity}
                </div>
              </div>
            </div>
          )}
          ...
          >
            ...
            <div className="rounded-xl border p-4">
              <div className="text-lg">
                💡 {recommendation || "Analysis ready."}
              </div>
              <div className="mt-2 text-sm text-gray-600">
                🎉 Smart Farmer! You completed soil & crop scan.
              </div>
            </div>
          </section>
        )}
