...
export default function Login() {
  ...
  const onSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
    ...
    setLoading(true);
    if (supports)
      speak("Logging in, please wait", { lang: `${lang}-IN` as any });
    setTimeout(() => {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(ellipse at bottom, rgba(255,200,0,.15), transparent 60%)",
        }}
      />
        ...
        <div className="w-full max-w-md rounded-3xl border bg-white/30 backdrop-blur-xl shadow-[0_10px_40px_rgba(46,125,50,.25)] animate-pop-in">
          <div className="p-6 pb-2 text-center">
            ...
            <button
              onClick={() =>
                supports &&
                speak("Enter your phone and password", {
                  lang: `${lang}-IN` as any,
                })
              }
          ...
          <form onSubmit={onSubmit} className="p-6 pt-2">
            ...
            <div className="mt-1 flex items-center gap-2 rounded-xl border bg-white/70 px-3 py-2 shadow-inner">
              ...
              <button
                type="button"
                onClick={() => setShow((s) => !s)}
                className="text-sm text-emerald-700 focus:outline-none"
              >
                {show ? "Hide" : "Show"}
              </button>
            </div>
            <button
              disabled={loading}
              className="mt-6 w-full rounded-2xl py-3 text-white font-semibold shadow-lg hover:shadow-xl transition-all"
            ...
            <div className="mt-4 flex items-center justify-between text-sm">
              <button
                type="button"
                className="px-3 py-2 rounded-xl border bg-white/70 hover:shadow"
              >
                🔓 OTP
              </button>
              <button
