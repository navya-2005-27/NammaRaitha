...
export default function Settings() {
  ...
  const announce = (msg: string) => {
    if (voiceEnabled && supports)
      speak(msg, { lang: `${voiceLang}-IN` as any });
  };
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="border-b bg-gradient-to-r from-emerald-700 to-green-700 text-white">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden sm:flex items-center gap-4 text-sm opacity-90">
            <a href="/dashboard?lang=" className="hover:underline">
              Dashboard
            </a>
            <a href="/reports?lang=" className="hover:underline">
              Reports
            </a>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="rounded-2xl border bg-white/80 backdrop-blur p-5">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              {
                key: "soilMoisture",
                label: "Soil Moisture Sensors",
                sub: "4 devices connected",
              },
              ...
              },
              {
                key: "fieldCameras",
                label: "Field Cameras",
                sub: "6 devices connected",
              },
              {
                key: "npk",
                label: "NPK Nutrient Sensors",
                sub: "Pending setup",
              },
              {
                key: "autoIrrigation",
                label: "Auto Irrigation System",
                sub: "Smart sprinkler control",
              },
            ].map((it) => (
              <div
                key={it.key}
              ...
              >
                ...
                <label className="inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    className="sr-only peer"
                    checked={(sensors as any)[it.key]}
                    onChange={(e) => {
                      const v = e.target.checked;
                      setSensors((s) => ({ ...s, [it.key]: v }) as any);
                      announce(`${it.label} ${v ? "enabled" : "disabled"}`);
                    }}
                  />
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            <label className="rounded-xl border p-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-gray-600">
                  Disease detection, pest outbreaks
                </div>
              </div>
              <input
                type="checkbox"
                ...
                onChange={(e) => {
                  setNotifCritical(e.target.checked);
                  announce(
                    "Critical alerts " + (e.target.checked ? "on" : "off"),
                  );
                }}
              />
            </label>
            <label className="rounded-xl border p-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-gray-600">
                  Rain, frost, extreme temperature
                </div>
              </div>
              <input
                type="checkbox"
                ...
                onChange={(e) => {
                  setNotifWeather(e.target.checked);
                  announce(
                    "Weather alerts " + (e.target.checked ? "on" : "off"),
                  );
                }}
              />
            </label>
            <label className="rounded-xl border p-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-gray-600">
                  Flight status, image capture progress
                </div>
              </div>
              <input
                type="checkbox"
                ...
                checked={notifDrone}
                onChange={(e) => {
                  setNotifDrone(e.target.checked);
                  announce(
                    "Drone updates " + (e.target.checked ? "on" : "off"),
                  );
                }}
              />
            </label>
        ...
        <div className="flex items-center justify-end gap-3">
          <button
            onClick={onReset}
            className="px-4 py-2 rounded-lg border bg-white hover:shadow"
          >
            Revert to Defaults
          </button>
          <button
