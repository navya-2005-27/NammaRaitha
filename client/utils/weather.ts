export type Weather = { temperature: number | null; humidity: number | null; precipitation: number | null; name?: string };
...
export async function reverseGeocode(lat: number, lon: number): Promise<string | undefined> {
  try {
    const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lon}`, { headers: { 'User-Agent': 'agri-scan-ai-demo' } });
    const data = await res.json();
    return data?.address?.village || data?.address?.hamlet || data?.display_name;
  } catch {
    return undefined;
  }
}
